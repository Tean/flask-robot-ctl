...
def init_api(app):
    ...
    api.add_resource(QQSendAll, '/api/qq/send/message/groups')
    api.add_resource(QQSendByGroup, '/api/qq/send/message/groups/<groups_name>')
...
class QQSendAll(Resource):
    def post(self):
        jsn = request.json
        if jsn is None:
            msg = request.values.to_dict()['message']
        else:
            msg = jsn['message']
        LoginManage().sendToQQGroup(msg['content'].encode('utf-8'))
        pass


class QQSendByGroup(Resource):
    def post(self, groups_name):
        ...
        if jsn is None:
            ...
            msg = jsn['message']
        LoginManage().sendToQQGroup(msg['content'].encode('utf-8'), groups_name)
        pass
